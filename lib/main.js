...
function Cosine() {
	...
	if ( arguments.length ) {
		...
		if ( !isNumber( mu ) || isnan( mu ) ) {
			throw new TypeError( format( '0xb8p,HM', mu ) );
		}
		if ( !isPositive( s ) ) {
			throw new TypeError( format( '0xb7c,HN', s ) );
		}
	} else {
		mu = 0.0;
		s = 1.0;
	...
	defineProperty( this, 'mu', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '0xb8d,H8', value ) );
			}
			mu = value;
		}
	});
	defineProperty( this, 's', {
		...
		'set': function set( value ) {
			if ( !isPositive( value ) ) {
				throw new TypeError( format( '0xb8k,HE', value ) );
			}
			s = value;
		}
	});
